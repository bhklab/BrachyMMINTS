...
class SegmentationNetwork(NeuralNetwork):
    ...
    def predict_3D(self, x: np.ndarray,  do_mirroring: bool, mirror_axes: Tuple[int, ...] = (0, 1, 2),
                   ...
                   verbose: bool = True, mixed_precision: bool = True, mcdo: int = -1, text=None) -> Tuple[np.ndarray, np.ndarray]:
        """
    ...
    def _internal_predict_3D_3Dconv_tiled(self, x: np.ndarray, step_size: float, do_mirroring: bool, mirror_axes: tuple,
                                          ...
                                          verbose: bool, text=None) -> Tuple[np.ndarray, np.ndarray]:
            ...
            for y in steps[1]:
                ...
                for z in steps[2]:
                    ...
                    predicted_patch = self._internal_maybe_mirror_and_pred_3D(
                        ...
                        gaussian_importance_map, text=text)[0]
    ...
    def _internal_maybe_mirror_and_pred_3D(self, x: Union[np.ndarray, torch.tensor],  mirror_axes: tuple,
                                           ...
                                           mult: np.ndarray or torch.tensor = None, text = None) -> torch.tensor:
        ...
        if text is not None:
            text = maybe_to_torch(text)
            if torch.cuda.is_available():
                text = to_cuda(text, gpu_id=self.get_device())
        ...
        if text == None:
            ...
            for m in range(mirror_idx):
                if m == 0:
                    if isinstance(self(x, text), tuple):
                        pred = self.inference_apply_nonlin(self(x, text)[0])
                        result_torch += 1 / num_results * pred
                    else:
                        pred = self.inference_apply_nonlin(self(x, text))
                        result_torch += 1 / num_results * pred

                if m == 1 and (2 in mirror_axes):
                    pred = self.inference_apply_nonlin(self(torch.flip(x, (4, )), text))
                    result_torch += 1 / num_results * torch.flip(pred, (4,))

                if m == 2 and (1 in mirror_axes):
                    pred = self.inference_apply_nonlin(self(torch.flip(x, (3, )), text))
                    result_torch += 1 / num_results * torch.flip(pred, (3,))

                if m == 3 and (2 in mirror_axes) and (1 in mirror_axes):
                    pred = self.inference_apply_nonlin(self(torch.flip(x, (4, 3)), text))
                    result_torch += 1 / num_results * torch.flip(pred, (4, 3))

                if m == 4 and (0 in mirror_axes):
                    pred = self.inference_apply_nonlin(self(torch.flip(x, (2, )), text))
                    result_torch += 1 / num_results * torch.flip(pred, (2,))

                if m == 5 and (0 in mirror_axes) and (2 in mirror_axes):
                    pred = self.inference_apply_nonlin(self(torch.flip(x, (4, 2)), text))
                    result_torch += 1 / num_results * torch.flip(pred, (4, 2))

                if m == 6 and (0 in mirror_axes) and (1 in mirror_axes):
                    pred = self.inference_apply_nonlin(self(torch.flip(x, (3, 2)), text))
                    result_torch += 1 / num_results * torch.flip(pred, (3, 2))

                if m == 7 and (0 in mirror_axes) and (1 in mirror_axes) and (2 in mirror_axes):
                    pred = self.inference_apply_nonlin(self(torch.flip(x, (4, 3, 2)), text))
                    result_torch += 1 / num_results * torch.flip(pred, (4, 3, 2))
