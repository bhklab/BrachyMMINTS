...
def predict_cases(model, list_of_lists, output_filenames, folds, save_npz, num_threads_preprocessing,
    ...
    for preprocessed in preprocessing:
        ...
        case_name = output_filename.split('/')[-1].split('.')[0]
        # df = pd.read_pickle('/cluster/projects/radiomics/InternalDatasets/procdata/Pelvis/PMCC_Gyn_Autosegmentation/GTV_HRCTV_seg/HRCTV_notes_seg/text/llama3-Sep/Pubmedbert_processed_Sep20-try2-vag.pkl')
        df = pd.read_pickle('/cluster/projects/radiomics/InternalDatasets/procdata/Pelvis/PMCC_Gyn_Autosegmentation/GTV_HRCTV_seg/HRCTV_notes_seg/text/llama3-siglip-25Jun/siglip_processed_Sep20-try2-vag.pkl')
        df_1 = df[df['nnUNetID'] == case_name]
        # notes = np.array(df_1['LLM_output'].tolist())

        # for encoded
        # df = pd.read_pickle('/cluster/projects/radiomics/Gyn_Autosegmentation/nnUNetFrame/DATASET/nnUNet_raw/nnUNet_raw_data/Task070_gyn/text/encoded_votes_Aug21.pkl')
        # df_1 = df[df['nnUNetID'] == case_name] # randomly choose one from topk rows

        # RANDOM selection
        select_column = random.choice(df.filter(like='summary').columns)
        # select_column = 'summary1_output'
        notes = np.array(df_1[select_column].tolist())

        # POP selection
        # Path to a file where the list of remaining summary columns is stored
        # columns_path = '/cluster/projects/radiomics/Gyn_Autosegmentation/nnUNetFrame/DATASET/nnUNet_raw/nnUNet_raw_data/Task091_gyn/text/Pubmedbert_processed_50-columnname.pkl'
        # Try to load the list of remaining summary columns; if not found or empty, initialize it
        # try:
        #     with open(columns_path, 'rb') as f:
        #         summary_columns = pickle.load(f)
        #     if not summary_columns:  # If the list is empty, reinitialize it
        #         raise FileNotFoundError
        # except (FileNotFoundError, EOFError):  # EOFError catches empty file
        #     summary_columns = df.filter(like='summary_').columns.tolist()
        # # Randomly select and pop a column from the list
        # selected_index = random.randint(0, len(summary_columns) - 1)
        # select_column = summary_columns.pop(selected_index)
        # # Save the updated list back to the file
        # with open(columns_path, 'wb') as f:
        #     pickle.dump(summary_columns, f)
        # notes = np.array(df_1['Encoded'].tolist())


        print("predicting", output_filename)
        trainer.load_checkpoint_ram(params[0], False)

        ## Ensemble (average all 50 predictions)
        # softmax_outputs = []
        # summary_columns = df.filter(like='summary_').columns
        # for column in summary_columns:
        #     # Extract notes for the current column
        #     notes = np.array(df_1[column].tolist())

        # Original code starts
        softmax = trainer.predict_preprocessed_data_return_seg_and_softmax(
            d, text=notes, do_mirroring=do_tta, mirror_axes=trainer.data_aug_params['mirror_axes'], use_sliding_window=True,
        ...
        # stacked_outputs = np.stack(softmax_outputs)
        # # Average 50 predictions
        # softmax = np.mean(stacked_outputs, axis=0)
